...
export const getOne = (req, res) => {
    ...
    try {
        const q = `select * from devis where devis_id = ? `;
...
export const postRow = (req, res) => {
    ...
    db.query(q, data, (err, data) => {
        if (err) return res.status(500).send(err);
        return res.status(201).json(`devis has been created with successfull!`);
    })
    } catch (error) {
        console.log(error);
    }
...
export const updateRow = (req, res) => {
    ...
    const data = req.body;
    try {
        const q = `update devis set ? where devis_id = ? `;
    db.query(q, [data, id], (err, data) => {
        if (err) return res.status(500).send(err);
        return res.status(201).json(`devis has been updated with successfull!`);
    })
    } catch (error) {
        console.log(error);
    }
...
export const deleteRow = (req, res) => {
    const id = req.params.id;
    try {
        const q = `update devis set devis_status = ? where devis_id = ? `;
    db.query(q, ['0', id], (err, data) => {
        if (err) return res.status(500).send(err);
        return res.status(201).json(`devis has been deleted with successfull!`);
    })
    } catch (error) {
        console.log(error);
    }
...
export const recoverRow = (req, res) => {
    ...
    db.query(q, ['1', id], (err, data) => {
        if (err) return res.status(500).send(err);
        return res.status(201).json(`devis has been recovred with successfull!`);
    })
    } catch (error) {
        console.log(error);
    }
